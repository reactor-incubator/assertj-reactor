...
class Tuple5AssertTest {
	...
	final Tuple5<String, Integer, Long, String, Integer>       testData  = Tuples.of("example", 123, 45L, "fourth", 5);
	final Tuple5Assert<String, Integer, Long, String, Integer> assertion = ReactorAssertions.assertThat(testData);
	...
	void hasT1Fails() {
		...
		).withMessage("Expected Tuple5 to have first part <examples> but was <example>");
	}
	...
	void hasT2Fails() {
		...
		).withMessage("Expected Tuple5 to have second part <1234> but was <123>");
	}
	...
	void hasT3Fails() {
		assertThatExceptionOfType(AssertionError.class).isThrownBy(() ->
				assertion.hasT3(46L)
		).withMessage("Expected Tuple5 to have third part <46> but was <45>");
	}
	...
	void hasT4Fails() {
		...
		).withMessage("Expected Tuple5 to have fourth part <foorth> but was <fourth>");
	}
